...
export function ChatLayout({
  ...
  const sendMessage = (e: React.FormEvent) => {
    ...
    const newMsg: ChatMessage = {
      id: `msg-${activeConv.id}-${activeConv.messages.length + 1}`,
      ...
      {activeConv ? (
        <div className="flex flex-1 flex-col min-w-0">
          ...
          <div className="flex items-center gap-3 border-b border-gray-200 bg-white px-5 py-4">
            {(() => {
              const other =
                activeConv.participants.find((p) => p.id !== currentUserId) ??
          ...
          {isMonitoring && (
            <div className="border-b border-violet-100 bg-violet-50 px-5 py-2 text-xs text-violet-700">
              ...
              {activeConv.candidateName} と {activeConv.companyName}{" "}
              の直接チャットを監視しています。
            </div>
          )}
              ...
              return (
                <div key={msg.id}>
                  ...
                  <MessageBubble
                    ...
                    senderColor={
                      senderParticipant?.avatarColor ?? "bg-gray-400"
                    }
                  />
                </div>
              );
