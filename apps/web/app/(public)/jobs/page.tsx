...
import { toMockJob } from "@/lib/api/job-adapter";
...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    void api
      .searchJobs(params)
      .then((result) => setJobs(result.items.map(toMockJob)))
      .catch((requestError) =>
        setError(
  ...
  const filtered = jobs.filter((job) => {
    ...
    if (
      ...
      job.prefecture !== selectedPrefecture
    )
      return false;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        <div className="flex gap-8">
          ...
          <main className="min-w-0 flex-1">
            ...
            {error ? (
              <div className="rounded-xl border border-red-100 bg-red-50 py-16 text-center text-sm text-red-700">
                {error}
              </div>
            ) : isLoading ? (
