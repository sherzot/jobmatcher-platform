...
const STATS = [
  ...
  {
    label: "面接予定",
    value: MOCK_APPLICATIONS.filter((a) => a.status.includes("INTERVIEW"))
      .length,
  ...
  {
    label: "不採用",
    value: MOCK_APPLICATIONS.filter((a) => a.status === "REJECTED").length,
    color: "text-red-500",
    bg: "bg-red-50",
  },
];
...
export default function DashboardPage() {
  ...
  const recommendedJobs = MOCK_JOBS.filter(
    (j) => j.japaneseLevel === "NONE" || j.japaneseLevel === "N2",
  ).slice(0, 3);
    ...
    <div className="p-6 lg:p-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800">
                プロフィールが{MOCK_USER.profileCompleteness}%完成しています
              </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-indigo-600">
                {nextAction.nextAction} — {nextAction.companyName}
              </p>
            </div>
          </div>
          <Link
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {STATS.map((stat) => (
          <div
            key={stat.label}
            className={cn("rounded-xl border border-gray-100 p-4", stat.bg)}
          >
          ...
          <div className="divide-y divide-gray-50">
            {recommendedJobs.map((job) => (
              <Link
                key={job.id}
                href={`/jobs/${job.id}`}
                className="group block px-5 py-3.5"
              >
                <div className="flex items-start gap-3">
                  <div
                    className={cn(
      ...
      <div className="mt-6 rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {MOCK_USER.skills.map((skill) => (
            <span
              key={skill}
              className="rounded-lg bg-indigo-50 px-3 py-1 text-sm font-medium text-indigo-700"
            >
              {skill}
            </span>
          ))}
