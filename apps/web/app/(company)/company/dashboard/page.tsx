...
export default function CompanyDashboardPage() {
  ...
  const newApplications = MOCK_COMPANY_JOBS.reduce(
    (s, j) => s + j.newApplicationCount,
    0,
  );
  const unreadMessages = MOCK_MESSAGES.filter(
    (m) => !m.isRead && m.senderId === "agent",
  ).length;
  const lastMessage = [...MOCK_MESSAGES]
    .reverse()
    .find((m) => m.senderId === "agent");

  const STATS = [
    {
      label: "公開中求人",
      value: activeJobs.length,
      sub: `全${MOCK_COMPANY_JOBS.length}件`,
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {STATS.map((s) => (
          <Link key={s.label} href={s.href}>
            <div
              className={cn(
                "rounded-xl border border-gray-100 p-4 transition-all hover:shadow-sm",
                s.bg,
              )}
            >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="divide-y divide-gray-50">
            {MOCK_COMPANY_JOBS.map((job) => (
              <Link
                ...
                className="group flex items-center gap-4 px-5 py-4 hover:bg-gray-50"
              >
                <div className="min-w-0 flex-1">
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900">
                  {MOCK_ASSIGNED_AGENT.displayName}
                </p>
          ...
          <div className="rounded-xl border border-gray-200 bg-white">
            ...
            <div className="divide-y divide-gray-50">
              {MOCK_COMPANY_APPLICATIONS.filter((a) => a.status !== "REJECTED")
                .slice(0, 3)
                .map((app) => (
