...
function AgentFormModal({
  ...
  const set = (k: keyof AgentFormData, v: string) =>
    setForm((f) => ({ ...f, [k]: v }));
...
export default function AdminAgentsPage() {
      ...
      .then((records) =>
        setAgents(
          records.map((agent, index) => ({
            ...
            phone: agent.phone ?? "—",
            avatarInitial: agent.displayName.slice(0, 1),
            avatarColor: ["bg-[#1456d9]", "bg-[#c9233f]", "bg-[#f0b429]"][
              index % 3
            ],
            status: agent.isActive ? "ACTIVE" : "INACTIVE",
            assignedCompanies: agent.agentCompanies?.length ?? 0,
            activeJobs: 0,
            totalCandidates: 0,
            applicationsThisMonth: 0,
            offersMadeTotal: 0,
            joinedAt: new Date(agent.createdAt).toLocaleDateString("ja-JP"),
          })),
        ),
      )
      .catch((error) =>
  ...
  const handleSave = async (data: AgentFormData) => {
    ...
    if (modal.mode === "create") {
      try {
        await api.createAdminAgent({
          email: data.email,
          password: data.password ?? "",
          displayName: data.displayName,
          phone: data.phone,
        });
        const records = await api.adminAgents();
        setAgents(
          records.map((agent, index) => ({
            id: String(agent.id),
            code: agent.agentCode,
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-400">
            {agents.length}名 — アクティブ {activeCount} · 非アクティブ{" "}
            {inactiveCount}
          </p>
        </div>
        <button
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-gray-100 bg-gray-50">
            <tr>
              ...
              <th className="hidden px-4 py-3 text-center text-xs font-semibold text-gray-500 sm:table-cell">
                担当企業
              </th>
              <th className="hidden px-4 py-3 text-center text-xs font-semibold text-gray-500 md:table-cell">
                求人
              </th>
              <th className="hidden px-4 py-3 text-center text-xs font-semibold text-gray-500 lg:table-cell">
                候補者
              </th>
              <th className="hidden px-4 py-3 text-center text-xs font-semibold text-gray-500 lg:table-cell">
                今月応募
              </th>
              <th className="hidden px-4 py-3 text-center text-xs font-semibold text-gray-500 xl:table-cell">
                内定数
              </th>
              <th className="hidden px-4 py-3 text-left text-xs font-semibold text-gray-500 xl:table-cell">
                参加日
              </th>
              ...
              >
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center justify-end gap-2">
                    <button
                      onClick={() =>
                        setModal({ mode: "edit", agentId: agent.id })
                      }
