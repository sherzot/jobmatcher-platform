...
export default function AdminCompaniesPage() {
  const [companies, setCompanies] =
    useState<AdminCompany[]>(MOCK_ADMIN_COMPANIES);
      ...
      .then(({ items }) =>
        setCompanies(
          items.map((company) => ({
            ...
            name: company.name,
            nameEn: company.name,
            logoInitial: company.name.slice(0, 1),
            logoColor: "bg-[#1456d9]",
            industry: company.industry ?? "—",
            prefecture: company.prefecture ?? "—",
            employeeCount: company.employeeCount ?? "—",
            assignedAgentId:
              company.agentCompanies?.[0]?.agent.agentCode ?? "—",
            assignedAgentName:
              company.agentCompanies?.[0]?.agent.displayName ?? "未割当",
            verificationStatus: (company.status === "APPROVED"
              ? "VERIFIED"
              : company.status === "REJECTED"
                ? "REJECTED"
                : "PENDING") as CompanyVerificationStatus,
            isActive: company.isActive,
            activeJobCount: 0,
            totalApplications: 0,
            registeredAt: new Date(company.createdAt).toLocaleDateString(
              "ja-JP",
            ),
          })),
        ),
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {company.code} · {company.prefecture}
                        </p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4 text-center">
                    <span
                      className={cn(
                        ...
                        VERIFICATION_COLORS[company.verificationStatus],
                      )}
                    >
                    ...
                    <div className="flex items-center justify-end gap-2">
                      {company.verificationStatus === "PENDING" && (
                        <>
                          <button
                            onClick={() =>
                              setVerification(company.id, "VERIFIED")
                            }
                          ...
                          <button
                            onClick={() =>
                              setVerification(company.id, "REJECTED")
                            }
