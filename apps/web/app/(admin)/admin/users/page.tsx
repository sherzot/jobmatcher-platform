...
export default function AdminUsersPage() {
      ...
      .then(({ items }) =>
        setUsers(
          items.map((user, index) => ({
            ...
            lastName: user.profile?.lastName ?? user.role,
            nationality: "—",
            visaType: "—",
            japaneseLevel: "NONE",
            ...
            registeredAt: new Date(user.createdAt).toLocaleDateString("ja-JP"),
            lastLoginAt: "—",
            avatarInitial: (user.profile?.firstName ?? user.role).slice(0, 1),
  ...
  const toggleStatus = async (
    ...
    setApiError(null);
    try {
      await api.updateAdminUserStatus(code, next);
      setUsers((prev) =>
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() =>
                          void toggleStatus(user.id, user.code, user.status)
                        }
