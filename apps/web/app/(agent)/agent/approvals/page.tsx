...
import { PendingCompany } from "@/lib/mock/pending-companies";
...
export default function AgentApprovalsPage() {
      ...
      .then((records) => {
        ...
        setCompanies(
          records.map(({ company }) => ({
            ...
            businessRegNumber: company.businessRegNumber ?? undefined,
            registrationNote: company.registrationNote ?? undefined,
            email: "—",
            registeredAt: company.createdAt,
            status: company.status as PendingCompany["status"],
  ...
  const handleApprove = async () => {
    if (!selected) return;
    try {
      await api.reviewCompany(selected.companyCode, "approve");
      setCompanies((prev) =>
        prev.map((c) =>
  ...
  return (
    <div className="p-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 py-16 text-center text-sm text-red-700">
          {loadError}
        </div>
      ) : filtered.length === 0 ? (
                    ...
                    {company.status === "REJECTED" &&
                      company.rejectionReason && (
                        <div className="mt-2 rounded-lg bg-red-50 p-3 text-xs text-red-600">
                          <p className="font-semibold">否認理由：</p>
                          {company.rejectionReason}
                        </div>
                      )}
                  </div>
      ...
      {modal === "approve" && selected && (
        <Modal>
          ...
          <p className="mb-1 text-sm text-gray-600">
            ...
            を承認すると、この企業はログインして採用活動を開始できます。
          </p>
          <p className="mb-6 text-xs text-gray-400">
            承認後、企業のメールアドレスに通知が送られます。
          </p>
