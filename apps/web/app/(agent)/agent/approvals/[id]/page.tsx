...
import type { PendingCompany } from "@/lib/mock/pending-companies";
import { api } from "@/lib/api/client";

export default function CompanyDetailPage() {
  const { id } = useParams<{ id: string }>();
  const [company, setCompany] = useState<PendingCompany | null>(null);
  const [status, setStatus] =
    useState<PendingCompany["status"]>("PENDING_APPROVAL");
    ...
    api
      ...
      .then((records) => {
        const record = records.find(
          ({ company: item }) => String(item.id) === id,
        );
        if (!active) return;
        if (!record) {
          setLoadError("企業が見つかりません");
          return;
        }
        const item = record.company;
        const mapped: PendingCompany = {
          id: String(item.id),
          ...
          businessRegNumber: item.businessRegNumber ?? undefined,
          registrationNote: item.registrationNote ?? undefined,
          email: "—",
          registeredAt: item.createdAt,
          status: item.status as PendingCompany["status"],
  ...
  if (!company || loadError) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-6 text-center">
        <p className="text-gray-400">{loadError || "企業が見つかりません"}</p>
        <Link
  ...
  const handleApprove = async () => {
    ...
    try {
      await api.reviewCompany(company.companyCode, "approve");
      setStatus("APPROVED");
      setModal(null);
    } catch (error) {
      window.alert(
        error instanceof Error ? error.message : "審査に失敗しました",
      );
    } finally {
  ...
  const handleReject = async () => {
    ...
    setIsProcessing(true);
    try {
      await api.reviewCompany(
        company.companyCode,
        "reject",
        rejectInput.trim(),
      );
      setStatus("REJECTED");
      setRejectionReason(rejectInput.trim());
      setModal(null);
    } catch (error) {
      window.alert(
        error instanceof Error ? error.message : "審査に失敗しました",
        ...
        <Modal onClose={() => !isProcessing && setModal(null)}>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={!rejectInput.trim() || isProcessing}
