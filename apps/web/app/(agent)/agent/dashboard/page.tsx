...
const STATS = [
  ...
  {
    label: "選考中候補者",
    value: MOCK_AGENT_CANDIDATES.filter(
...
export default function AgentDashboardPage() {
  const recentCandidates = MOCK_AGENT_CANDIDATES.filter(
    (c) => !["REJECTED", "WITHDRAWN"].includes(c.status),
  ).slice(0, 4);

  const activeJobs = MOCK_AGENT_JOBS.filter((j) => j.status === "ACTIVE")
  ...
  return (
    <div className="p-6 lg:p-10">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#1456d9]">
            Agent workspace
          </p>
          <h1 className="text-2xl font-semibold tracking-[-0.03em] text-[#10233f]">
        ...
        <Link
          href="/agent/jobs/new"
          className="flex items-center gap-2 rounded-full bg-[#1456d9] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#0f46b5]"
        >
          <svg
            className="h-4 w-4"
            fill="none"
          ...
          <Link key={stat.label} href={stat.href}>
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span className={cn("rounded-lg p-2", stat.bg, stat.color)}>
                  {stat.icon}
                </span>
              </div>
            </div>
          ...
          <div className="divide-y divide-gray-50">
            {activeJobs.map((job) => (
              <Link
                ...
                className="group flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50"
              >
                <div
                  className={cn(
        ...
        <div className="grid grid-cols-1 divide-y divide-gray-50 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          {MOCK_AGENT_COMPANIES.map((company) => (
            <Link
              key={company.id}
              href="/agent/companies"
              className="group flex items-center gap-3 px-5 py-4 hover:bg-gray-50"
            >
              <div
                className={cn(
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  求人 {company.activeJobCount}件 · 応募{" "}
                  {company.totalApplications}件
                </p>
                {!company.isVerified && (
