...
import { LiveAgentJobDetail } from "@/components/agent/LiveAgentJobDetail";
...
export default async function AgentJobDetailPage({ params }: Props) {
  ...
  if (id.startsWith("J")) {
    return <LiveAgentJobDetail jobCode={id} />;
  }
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <nav className="mb-4 flex items-center gap-2 text-sm text-gray-400">
        <Link href="/agent/jobs" className="hover:text-gray-600">
          求人管理
        </Link>
        <span>/</span>
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">
              <Link href={`/agent/jobs/${id}/edit`}>編集</Link>
            </button>
            {job.status === "ACTIVE" && (
      ...
      <LiveApplications jobCode={job.code} />
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {candidate.prefecture} · 経験{" "}
                          {candidate.yearsOfExperience}年 · 日本語{" "}
                          {JAPANESE_LEVEL_LABELS[candidate.japaneseLevel]}
                        </p>
                      </div>
                      <div className="flex shrink-0 items-center gap-2">
                        <span
